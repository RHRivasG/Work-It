...
func (s *RoutineService) Handle(c interface{}) (interface{}, error) {
	...
	case commands.CreateRoutine:
		...
		name, err := values.NewRoutineName(command.Name)
		errs = append(errs, err)

		userId, err := values.NewRoutineUserID(command.UserID)
		errs = append(errs, err)

		description, err := values.NewRoutineDescription(command.Description)
		errs = append(errs, err)
	...
	case commands.AddRoutineTraining:
		...
		trainingID, err := values.NewRoutineTrainingID(command.TrainingID)
		if err != nil {
			return nil, err
		}
